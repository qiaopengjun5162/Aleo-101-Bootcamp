...
// Store the last execution response for verification
...
async function localProgramExecution(program, aleoFunction, inputs) {
    ...
    const executionResponse = await programManager.run(
        ...
        aleoFunction,
        inputs,
        true, // proveExecution — 生成真正的零知识证明
        undefined, // imports
        { cacheKey: `${program}:${aleoFunction}` }, // keySearchParams
    );

    // Store for later verification
    lastExecutionResponse = executionResponse;

    return executionResponse.getOutputs();
}

async function verifyExecutionProof(blockHeight) {
    if (!lastExecutionResponse) {
        throw new Error("没有可验证的证明，请先生成证明");
    }

    const programManager = new ProgramManager();
    const proofIsValid = programManager.verifyExecution(
